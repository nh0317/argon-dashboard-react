...
const StoreEdit = () => {
  ...
  const [br, onChangeBr, setBr] = useInput();
  const [ifNewBr, setIfNewBr]=useState(false);
  ...
  const onRoom = e => {
    ...
    if(room==null || room=="" || roomCount==null||roomCount==0)
      alert("방 정보를 입력해주세요!");
    else{
    axios.post("/partner/room", newData).then(response => {
      setReload(reload + 1);
      setRoom(""); setRoomCount(0);
    });
  }
  }
  const onDelRoom = e => {
      ...
      <Container className="mt--7" fluid>
              ...
              <Table className="align-items-center table-flush" responsive>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      <Button
                        color={!postPop?undefined:"primary"}
                        onClick={() => {
                          setPostPop(!postPop)
                        }}>
                        주소 검색
                  ...
                  <tr>
                    ...
                    <td>
                      <UncontrolledDropdown>
                        ...
                        >
                          {ifNewBr==false?br?br : "브랜드 선택":"새 브랜드 추가"}
                        ...
                        >
                          {brands.map(b => <DropdownItem
                            value={b.brandName}
                            onClick={e => {setBr(e.target.value);setIfNewBr(false);}}
                          >{b.brandName}</DropdownItem>)}
                          <DropdownItem
                            onClick={e => {setIfNewBr(true);setBr(null);}}
                          >새 브랜드 추가</DropdownItem>
                        </DropdownMenu>
                      </UncontrolledDropdown>
                            {ifNewBr==true?
        ...
        <Row>
          <div className="col">
            <Card className="shadow">
              <CardHeader className="border-0">
                <h3 className="mb-0">매장 방 관리</h3>
              </CardHeader>
                {data.storeName == null ?
                  <h5 className="m-3">! 매장 정보를 먼저 등록해주세요</h5>
                  :
                  <Table className="align-items-center table-flush" responsive>
                    <thead className="thead-light">
                      <tr>
